...
def get_time_stamp(name=None):
    ...
    _ = datetime.now().strftime('%d_%I_%M_%S_%p_%f')
    if name:
        name = name + '_' + _
    else:
        name = _
...
class FigureManager:
    ...
    def grid(ax, factor=5, x_or_y='both', color='gray', alpha1=0.5, alpha2=0.25):
        if type(ax) in {list, List, np.ndarray}:
            for an_ax in ax:
                FigureManager.grid(an_ax, factor=5, x_or_y='both', color='gray')
            return None
...
def _size(self, units='mb'):
    factor = {'b': 1, 'kb': 1024, 'mb': 1024**2, 'gb': 1024**3}[units]
    return round(self.stat().st_size / factor, 1)


def _get_num(self, astring=None):
    if astring is None:
        astring = self.stem
...
def append(self, name):
    return self.parent.joinpath(self.stem + '_' + name + ''.join(self.suffixes))
...
def _move(self, new_path):
    # assert self.is_file()
    self = self.absolute()
    self.rename(new_path.absolute() / self.name)
...
def _copy(self, target=None, verbose=True):
    import shutil
    # assert self.is_file()
    if target is None:
...
def glob_custom(self, pattern='*', r=False, list_=True, files=True, folders=True, dotfiles=False, return_type=Path,
...
def find(self, *args, **kwargs):
    ...
    """
    return self.myglob(*args, **kwargs)[0]


def read_(self, reader=None, **kwargs):
    if reader is None:
        reader = getattr(Read, self.suffix[1:])
    return reader(str(self), **kwargs)


def temp(subfolder=None, loc="home"):
    if loc == "home":
        path = P.home() / "deephead_tmp"
        path.mkdir(exist_ok=True, parents=True)
    else:
        path = Path('tmp').relativity_transform()
...
def __str__(self):  # this method is used by other functions to get string representation
    return str(self)
...
Path.size = _size
Path.get_num = _get_num
Path.readit = read_
Path.myglob = glob_custom
Path.listdir = listdir
Path.find = find
Path.relativity_transform = relativity_transform
Path.create = create
Path.clean = clean
Path.delete = _delete
Path.copy = _copy
Path.move = _move
Path.rename_fname = renameit
Path.make_python_name = make_python_name
Path.__add__ = __add__
Path.__repr__ = __repr__
Path.trunk = trunk
...
def browse(path, depth=8, width=20):
    ...
    if depth > 0:
        ...
        tmp = [os.path.basename(i) for i in val_paths]
...
class Read:
    ...
    def mat(path, correct_dims=True, single_item=False):
        ...
        try:  # try the old version
            ...
            data = {}
            for item in f:
                tmp = np.array(f[item], order='F')  # Now you get the correct shape.
                if correct_dims:
                    n = len(tmp.shape)
                    arrangements = tuple(range(n - 2)) + (n - 1, n - 2)
                    tmp = tmp.transpose(arrangements)
                data[item] = tmp
            f.close()
        return data
...
class List(list):
    ...
    def apply(self, func, *args, lest=None, jobs=None, **kwargs):
        """
        :param jobs:
    ...
    def modify(self, func, lest=None):
        """This is more flexible than apply, or it serves different application. Can be used where you modify objects
        rather than having a function that returns results, hence the name of the method.
...
def run(name, asis=False):
    ...
    tmp = inspect.getfullargspec(name)
    arg_string = """"""
    # if isinstance(type(name), types.MethodType) else tmp.args
    for key, val in zip(tmp.args[1:], tmp.defaults):
